...
name: wiki
description: >-
  Operates a shared LLM and human knowledge-base wiki for a project
...
- **Wiki root:** `<PROJECT_ROOT>/wiki`
- **Schema:** `wiki/principles.md` — authoritative workflow reference (read it when you need procedural detail not covered here)
...
- **Scoped commit helper:** use `wiki/tools/skills/wiki/scripts/log_and_commit.sh` with the exact changed file list so unrelated repo changes are excluded
